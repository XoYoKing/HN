...
//		refreshLayout.setRefreshFooter(new BallPulseFooter(context).setSpinnerStyle(SpinnerStyle.Scale));
     ...
     * @param network_img 网络加载失败的图片
     */
    public static void hintView(Object obj, boolean isNetwork, View network, ImageView noData_img, ImageView network_img) {
        if (obj == null) {
            network.setVisibility(View.VISIBLE);
            if (isNetwork) {
                network_img.setVisibility(View.VISIBLE);
     ...
     * @param network_img 网络加载失败的图片
     */
    public static void hintView(String str, boolean isNetwork, View network, ImageView noData_img, ImageView network_img) {
